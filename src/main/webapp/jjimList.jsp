...
<body>
                    ...
                    <div class="collapse navbar-collapse justify-content-between" id="navbarCollapse">
                        ...
                        <div class="navbar-nav ml-auto py-0">
                           <%
                        	if(name==null){
                        		%>
                        		<a href="login.jsp?from=jjimList.jsp" class="nav-item nav-link">Login</a>
                        		<a href="signUp.jsp" class="nav-item nav-link">Register</a>
                        		<%	
                        	}else{
                        		%>
                        		<a href="jjimList.jsp" class="nav-item nav-link"><%=name%></a>
    ...
    <div class="container-fluid bg-secondary mb-5">
        <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 300px">
            <h1 class="font-weight-semi-bold text-uppercase mb-3">Likes List</h1>
            <div class="d-inline-flex">
                <p class="m-0"><a href="">Home</a></p>
                <p class="m-0 px-2">-</p>
                <p class="m-0">Likes List</p>
            </div>
        </div>
    </div>
    <!-- Page Header End -->
	...
	<div class="container-fluid pt-5">
	    <div class="text-center mb-4">
            <h2 class="section-title px-5"><span class="px-2">찜 리스트</span></h2>
        </div>
	    <table class="table table-bordered text-center mb-0">
	        <thead class="bg-secondary text-dark">
                ...
                <tbody class="align-middle">
                    <% 
                    // 좋아요 목록을 가져옴
                    ArrayList<ShopVO> likesList = dao.getLikesList();
                    
                    // 좋아요 목록이 비어 있는지 확인
                    if (likesList.isEmpty()) { 
                    %>
                    <tr>
                        <td colspan="5">좋아요한 상품이 없습니다.</td>
                    </tr>
                    <% 
                    } else { 
                        // 좋아요 목록이 비어 있지 않은 경우, 각 상품을 테이블에 표시
                        for (ShopVO item : likesList) { 
                    %>
                    <tr>
                        <td class="align-middle"><img src="<%=item.getImage()%>" alt="" style="width: 50px;"></td>
                        <td class="align-middle"><%=item.getName()%></td>
                        <td class="align-middle"><%=item.getContent()%></td>
                        <td class="align-middle"><%=item.getPrice()%>원</td>
                        <!-- detail 창으로 연결(이미지 더블클릭시 취소) -->
                        <td class="align-middle">
                            <button class="btn btn-sm btn-primary" onclick="location.href='detail.jsp?idx=<%=item.getCode()%>'">
